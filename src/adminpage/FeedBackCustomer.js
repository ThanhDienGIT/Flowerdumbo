 ...
 const API_KEY = "434853968532154";
  ...
  const handleDelete = async (id) => {
    ...
    const response = await axios.delete(
          `https://api.cloudinary.com/v1_1/${CLOUD_NAME}/auto/${id}`,
        );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log('Thông tin ảnh đã lưu:', uploadedImagesInfo);
...
const addFeedBack = async (feedback) => {
  try {
    ...
    const newPicture = {
        ...
        idPicture: feedback.data.idImage,
    }
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1200px', margin: 'auto' }}>
      ...
      <h2>Ảnh đã tải lên ({fileList.length})</h2>
