...
import { ref, onValue, push, set, remove, update, get, query, orderByChild, equalTo } from "firebase/database"; // Loại bỏ push, limitToLast, onValue, orderByChild vì không dùng trực tiếp trong các hàm này
...
const addFlower = async (flowerData) => {
  try {
    ...
    const newId = currentFlowers.length > 0 
...
const editFlower = async (id, updatedObject) => {
  try {
    ...
    if (snapshot.exists()) {
      ...
      const flowerIndex = currentFlowers.findIndex(flower => flower.id === id);
...
const deleteFlower = async (id) => {
  try {
    ...
    if (snapshot.exists()) {
      ...
      if (updatedFlowers.length < initialLength) { // Kiểm tra xem có phần tử nào bị xóa không
...
const getFlowerById = (flowerId) => {
  return new Promise((resolve, reject) => {
    ...
    get(flowerRef).then((snapshot) => {
      const data = snapshot.val();
      if (data) {
        resolve(FlowerDTO({ // Sử dụng FlowerDTO factory function
          id: flowerId,
          name: data.name,
      ...
      console.error(`Lỗi khi đọc dữ liệu hoa ${flowerId} từ Firebase:`, error);
      reject(error);
    });
  });
};
...
const getFlowersByCategoryId = (categoryId) => {
  return new Promise((resolve, reject) => {
    ...
    const categoryQuery = query(classificationRef, orderByChild('categoryId'), equalTo(categoryId));

    onValue(categoryQuery, async (snapshot) => { // Dùng async để có thể dùng await bên trong
      const data = snapshot.val();
      const flowerIds = [];
      if (data) {
        // Thu thập tất cả flowerId từ các mục phân loại có categoryId tương ứng
        for (let key in data) {
          if (data[key].flowerId) { // Đảm bảo flowerId tồn tại
      ...
      console.error(`Lỗi khi đọc dữ liệu FlowerClassification theo danh mục ${categoryId} từ Firebase:`, error);
      reject(error);
    });
  });
};
...
const getFlowersByStatus = (statusValue) => {
  return new Promise((resolve, reject) => {
    const flowersRef = ref(database, 'flower');
    const statusQuery = query(flowersRef, orderByChild('status'), equalTo(statusValue));
    ...
    onValue(statusQuery, (snapshot) => {
      ...
      console.error(`Lỗi khi đọc dữ liệu hoa theo trạng thái ${statusValue} từ Firebase:`, error);
      reject(error);
    });
  });
};
...
const getRelatedFlowers = async (categoryId, currentFlowerId) => {
  try {
    if (!categoryId) {
      console.log("FlowerAPI: categoryId không được cung cấp cho getRelatedFlowers.");
      return [];
    }
    ...
    const filteredFlowerIds = flowerIds.filter(id => id !== String(currentFlowerId));

    if (filteredFlowerIds.length === 0) {
      console.log(`FlowerAPI: Không tìm thấy hoa liên quan cho categoryId "${categoryId}" (ngoại trừ hoa hiện tại).`);
      return [];
    }
    ...
    console.log(`FlowerAPI: Đã tải ${relatedFlowers.length} hoa liên quan cho categoryId "${categoryId}".`);
...
export { getListFlower, addFlower, editFlower, deleteFlower, getFlowerById, getFlowersByCategoryId,getFlowersByStatus ,getRelatedFlowers};